...
**Shared library (`js/library.js`):** the songs map has a "📚 Biblioteca compartida" block that reads the same Google Apps Script the diagram generator uses (`LIBRARY_URL`, GET `?codi=` for the list and `?codi=&id=` for one song; the teachers' code is kept in `rockin.play.biblio.codi`). `specFromLibrary(song, entry)` turns a generator song (`estructura.parts[].compassos[].cells`, `temps`, `to`/`mode`, `estil`, `tempo`, `ordre`) into a path spec: one chord per bar (the one filling most of a split bar), `gameChord` maps the generator's spellings (`D-`, `Bº`, `A5`) to the game's, `gameStyle` maps the free-text style to a backing style by keyword, 2/4 is played as 4/4. The spec is stored with the custom paths under the id `biblio:<fileId>`, so opening a song again refreshes the path and keeps its stars.
