...
**"El joc t'espera" (`waitMission`, type `wait`):** the first mission after the lesson in world 1. No transport: the ball rests on each circle of three cards and waits for the root (any octave); white circles go by on their own after a second (playing there is marked as a silence mistake), and bars slide while the key is held ("mantén!" if it is let go). A `wait` mission never locks the next one (`unlocked`). The keyboard fades in steps with the band level: smaller at level 2 (`.small-keys`), hidden from 3 when the mission paints no keys; the level-up line says so.
